## Homework Assignment 02 (10/13)

This assignment is primarily focused on control statements and functions. **Arrays are not allowed**.  You will complete the assignment individually. You are not allowed to share your code with other students. The assignment is worth a total of 100 points. If you have any questions or need any help, please visit us during office hours and/or post questions on Piazza.
...
#### 7. `sum_even`
Write a program that reads an integer `0 < n < 2^32`, returns the sum of all **digits** in `n` that are divisible by `2`. For example, if `n = 341238` the output would be `14`, because it is the sum of `4 + 2 + 8`.
...
#### 9. `draw_triangle_3`
Write a program which takes three integers `0 < {a, b, c} <= 20` as input. The program should print the `+` character `a` times, then `a + c` times, then `a + c + c` times, and so on. This pattern should repeat until the line is at least `b` characters long. At that point, the pattern is repeated backwards. For example input `4 7 1` will output:
```
++++
+++++
++++++
